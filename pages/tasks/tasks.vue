	...
	<view class="task-container">
		...
		<view class="task-list">
		  ...
		  <template v-if="activeTab === 0">
			...
			>
			  ...
			  <view class="task-right">
				...
				<view 
				  class="task-action daily-action"
		  ...
		  <template v-if="activeTab === 2">
			<view 
			  v-for="(task, index) in timeLimitTasks" 
			  :key="index" 
			  class="task-item time-limit"
			>
			  <view class="task-info">
				<view class="task-header">
...
.tab-item {
  ...
  &.active {
    ...
    .tab-daily, .tab-growth, .tab-time-limit {
      font-weight: 600;
	  color: #fff;
    }
  }
...
.daily-icon, .growth-icon, .time-icon {
  width: 60rpx;
  height: 60rpx;
  border-radius: 50%;
  display: flex;
